...
export default function ProjectAsWorld({ project, isSelected, handleClick, selectionOpensWorld }: IProjectAsWorldStateProps) {
    ...
    return (
        ...
        >
            <Image src={project.icon} alt={`logo_${project.title}`} width={500} height={500} className={"w-20 h-20 md:w-24 md:h-24 lg:w-32 lg:h-32"}/>

            <div className={"w-20 h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 bg-white/50 absolute left-1 top-1 hidden group-hover:block"}>
                <div className={"w-full h-full flex items-center justify-center relative"}>
                    ...
                    <Image src={"/textures/play_gray.png"} alt={"play_gray"} width={500} height={500} className={"w-10 h-10 md:w-16 md:h-16"}/>
                </div>
            </div>

            <div className={"flex flex-col gap-2 w-full ml-2"}>
                <span className={"portrait:text-md landscape:text-lg md:text-xl lg:text-2xl text-white"}>{project.title}</span>
                <span className={"text-md md:text-lg lg:text-2xl text-white/50"}>{project.description.slice(0, 70)}...</span>
            </div>
