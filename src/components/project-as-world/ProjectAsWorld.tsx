...
export default function ProjectAsWorld({ project, isSelected, handleClick }: IProjectAsWorldStateProps) {
    return (
        <div
            ...
            onClick={handleClick}
        >
            <Image src={project.icon} alt={`logo_${project.title}`} width={500} height={500} className={"w-32 h-32"}/>

            <div className={"w-32 h-32 bg-white/50 absolute left-1 top-1 hidden group-hover:block"}>
            ...
            <div className={"flex flex-col gap-2 w-full ml-2"}>
                ...
                <span className={"text-2xl text-white/50"}>{project.description.slice(0, 80)}...</span>
            </div>
